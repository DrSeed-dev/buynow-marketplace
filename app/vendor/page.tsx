...
export default function VendorPage() {
  ...
  const stats = [
    { label: t("vendor.totalProducts"),  value: "24",                  icon: Package,     color: "#6366f1", bg: "#eef2ff" },
    { label: t("vendor.totalRevenue"),   value: convertPrice(1240000), icon: TrendingUp,  color: "#22c55e", bg: "#f0fdf4" },
    { label: t("vendor.ordersReceived"), value: "87",                  icon: ShoppingBag, color: "#f59e0b", bg: "#fffbeb" },
    { label: t("vendor.avgRating"),      value: "4.7",                 icon: Star,        color: "#ec4899", bg: "#fdf2f8" },
  ]
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest mb-1"
            style={{ color: "var(--color-brand-600)" }}>{t("vendor.dashboard")}</p>
          <h1 className="text-2xl sm:text-3xl font-black"
            style={{ color: "var(--foreground)" }}>{t("vendor.storeOverview")}</h1>
        </div>
        <Link
      ...
      <div className="rounded-2xl border overflow-hidden"
        ...
        <div className="flex items-center justify-between p-4 sm:p-5 border-b"
          ...
          <h2 className="font-black text-base sm:text-lg"
            style={{ color: "var(--foreground)" }}>{t("common.yourProducts")}</h2>
          <Link href="/vendor/products"
            className="text-sm font-semibold hover:opacity-70"
            style={{ color: "var(--color-brand-600)" }}>{t("common.manageAll")}</Link>
        </div>
            ...
            <div key={product.id} className="p-4 flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="px-2 py-0.5 rounded-full text-xs font-bold"
                  ...
                  {product.status === "active" ? t("common.active") : t("common.outOfStock")}
                </span>
                ...
                <tr key={product.id} className="border-t"
                  ...
                  <td className="px-5 py-4">
                    <span className="text-sm font-medium"
                      ...
                      {product.stock === 0 ? t("common.outOfStock") : product.stock}
                    </span>
                  </td>
                  <td className="px-5 py-4">
