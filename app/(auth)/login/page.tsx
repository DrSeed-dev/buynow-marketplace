...
export default function LoginPage() {
  ...
  async function onSubmit(data: LoginFormData) {
    try {
      ...
      login({ id: "user-1", name: "Oluwatoyin Olamide", email: data.email, role: "shopper" })
  ...
  async function handleSocialLogin(provider: "google" | "facebook") {
    ...
    // TODO backend: redirect to OAuth provider
    await new Promise((r) => setTimeout(r, 1500))
    login({
      id: `${provider}-user`,
    ...
    <>
      <div className="mb-8">
        <h1 className="text-2xl font-black tracking-tight"
          style={{ color: "var(--foreground)" }}>Welcome back</h1>
        <p className="mt-1.5 text-sm" style={{ color: "var(--muted-foreground)" }}>
          Don&apos;t have an account?{" "}
          <Link href="/register" className="font-semibold hover:opacity-80"
            style={{ color: "var(--color-brand-600)" }}>Create one free</Link>
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-xs font-bold px-2"
          style={{ color: "var(--muted-foreground)" }}>or sign in with email</span>
        <div className="flex-1 h-px" style={{ backgroundColor: "var(--border)" }} />
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link href="/forgot-password"
              ...
              style={{ color: "var(--color-brand-600)" }}>Forgot password?</Link>
          </div>
        ...
        <div className="flex items-center gap-3">
          <input id="rememberMe" type="checkbox" {...register("rememberMe")}
            className="w-4 h-4 rounded cursor-pointer accent-[var(--color-brand-600)]" />
